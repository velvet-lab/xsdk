...
{
    public override Task<bool> RemoveAsync(ObjectId primaryKey, CancellationToken token = default)
    {
        _logger.LogTrace("Remove Entity '{0}'", primaryKey);
        return ExecuteInternalAsync(col => col.DeleteAsync(BsonValueConverter.Convert(primaryKey)), true, token);
    }

    public override Task<bool> RemoveAsync(TEntity entity, CancellationToken token = default) => RemoveAsync(entity.Id, token);

    public override async Task<int> RemoveAsync(IEnumerable<ObjectId> primaryKeys, CancellationToken token = default)
    {
    ...
    public override Task<int> RemoveAsync(IEnumerable<TEntity> entities, CancellationToken token = default) =>
        RemoveAsync(entities.Select(x => x.Id), token);
